...
import { getNewsArticles, getArticleBySlug, getRelatedArticles } from '@/lib/articles';
import authors from '@/data/authors.json';
import TagPill from '@/components/articles/TagPill';
import AuthorBox from '@/components/articles/AuthorBox';
import MubbooTake from '@/components/articles/MubbooTake';
...
export default async function NewsArticlePage({ params }: PageProps) {
  ...
  const related = getRelatedArticles('news', slug, frontmatter.tags, 4);
        ...
        <section className="border-t border-gray-100 bg-gray-50 py-12 sm:py-16">
          <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
            ...
            <div className="mt-6 grid gap-6 sm:grid-cols-2">
              {related.map((a) => (
                <ArticleCard key={a.slug} article={a} type="news" />
              ))}
            </div>
          </div>
        </section>
